...
K: key => (_K = $D.querySelector('b[data-x="'+key+'"]')), _K: null,
...
/*************************************************************************************************\
\*******  APP ENTRY POINT (main)  ******************************************  [ $WN.LOAD ]  ******/
LOAD: e => {
	if(location.protocol == 'http:')
		location.protocol = 'https:';
	for(let k of Object.keys($WN))
		window[k[0]=='_'?k:('$'+k)] = $WN[k];
	_components.forEach(c => $WN[c].setup());
	$D.childNodes.forEach(c => c.nodeType==Node.COMMENT_NODE ? console.log(c.nodeValue.trim()) : null);
},

/*************************************************************************************************\
\*******  GLOBAL EVENTS (automatically hooked)  *****************************  [ $EVT.* ]  *******/
EVT: {
	setup: () => Array.from(Object.keys($EVT)).forEach(k => (
		('on'+k in $D && !$D.addEventListener(k, $EVT[k], {passive:false})) ||
		('on'+k in $W && !$W.addEventListener(k, $EVT[k], {passive:false}))
	)),

	mouseup: e => {
		let dataRef=null, m, x='', y='', ref='', refList=Object.keys(_clickMap), el=(e&&e.target?e.target:e);
...
DAT: {
	...
	guess: word => {
		if(!$isValidWord(word))
			return;
		$GUI.busy(true);
		$NET.write($DAT.GAME + '|' + word + '|' + Math.ceil($DAT.IDX/5));
	},
	guessResponse: (game, mask) => {
		$GUI.busy(true);
		$DAT.WORDS += $DAT.WORD;
		$DB.set('words', $DAT.WORDS);
		$DAT.MASKS += mask;
...
GUI: {
	...
	key: key => {
		...
		if(key.length==1 && $DAT.WORD.length < 5 && $DAT.IDX < 30) {
			$DAT.WORD += key;
			_T[$DAT.IDX].innerText = key.toUpperCase();
			$GUI.reset(_T[$DAT.IDX], 'w_letter_typed 0.5s normal forwards');
			$DAT.IDX++;
		}
		else if(key=='backspace' && $DAT.WORD.length) {
			_T[--$DAT.IDX].innerText = '';
			$GUI.reset(_T[$DAT.IDX], 'w_letter_untyped 0.5s normal forwards');
			$DAT.WORD = $DAT.WORD.slice(0,-1);
		}
		else if(key=='enter' && $isValidWord($DAT.WORD))
			$DAT.guess($DAT.WORD);
		else
			noKey = true;
		if(key && !noKey && $K(key)) {
			$GUI.reset(_K, 'w_key 1.5s normal forwards');
	...
	reveal: ms => {
		if($GUI.IDX >= $DAT.WORDS.length)
			return($GUI.busy($GUI.IDX >= 30));
		if($GUI.IDX > $DAT.IDX || !$T('I'))
			return;
		$GUI.busy(true);
		setTimeout(() => $GUI.reveal(ms), ms);
		const m=$DAT.MASKS[$GUI.IDX].toLowerCase(), l=$DAT.WORDS[$GUI.IDX], t=_T[$GUI.IDX], k=$K(l.toLowerCase());
		$GUI.reset(t, `w_letter_flip ${ms}ms normal forwards`);
		setTimeout(() => {
			if($GUI.IDX > $DAT.IDX || !$DAT.WORDS[$GUI.IDX])
				return($GUI.busy(false));
			$GUI.busy(true);
			t.innerText = $DAT.WORDS[$GUI.IDX].toUpperCase();
			if(m=='g' || (m=='y' && k.className!='w_g') || !k.className)
				k.className = 'w_'+m;
			t.className = 'w_'+m;
			if(++$GUI.IDX >= 30) {
...
NET: {
	...
	connect: () => {
		...
		$NET.WS.onerror = $NET.error;
		$NET.WS.onclose = $NET.close;
	},
	open: () => {
		$GUI.busy(false);
		$NET.connected = true;
	...
	read: e => {
		...
		switch(args[0]) {
			...
			case 'M':
				...
				break;
			case 'W':
				const msg = (args[2]==$DAT.PLAYER_ID?'You':$playerIdToName(args[2]))+' '+($DAT.GAME==$DAT.GAME_MAX?'won':'finished')+' #'+args[1]+' '+(args[4]?' in '+args[4]:'')+': '+args[3].split('').map(x=>'[g'+x.toUpperCase()+']').join('');
...
NFY: {
	...
	notify: msg => {
		if(!$isVisible() && typeof Notification != 'undefined' && Notification.permission == 'granted') {
			$NFY.NOTIFICATIONS.push(new Notification('Wordle.NOW! - Player Activity', {
				icon: $A('link') && Array.from(_A).find(l => l.rel=='icon').href,
				body: msg
			}));
		}
		else 
		...
		$D.title = msg;
		if($NFY.TIMEOUT)
			clearTimeout($NFY.TIMEOUT);
		$NFY.TIMEOUT = setTimeout(() => $D.title = $NFY.TITLE, 5000);
	},
	requestPermission: () => {
		if($NFY.ALLOWED !== null || typeof Notification == 'undefined')
			return;
...
DB: {
	DB:{},
	setup: () => $DB.DB = localStorage.getItem('wordle') ? JSON.parse(localStorage.getItem('wordle')) : {},

	set: (key, value, append) => {
		$DB.DB[key] = (append&&$DB.DB[key] ? $DB.DB[key] : '') + value;
		$DB.save();
	},
	get: key => typeof $DB.DB[key] == 'undefined' ? null : $DB.DB[key],
	remove: key => delete $DB.DB[key] && $DB.save(),
	save: () => localStorage.setItem('wordle', JSON.stringify($DB.DB)),
	clear: () => !localStorage.setItem('wordle', JSON.stringify($DB.DB={})) && !$DB.save()
},

/*************************************************************************************************\
\*******  MISCELLANEOUS HELPERS  ************************************************  [ $* ]  *******/
isVisible: el => (el ? $W.getComputedStyle(el).visibility : $D.visibilityState) == 'visible',
isValidWord: word => word && word.length==5 && $WN._words.indexOf(word) >= 0,
capitalize: str => str && str[0].toUpperCase() + str.slice(1),
playerIdToName: id => $capitalize(_adjectives[parseInt(id,16)%_adjectives.length]) + $capitalize(_nouns[parseInt(id,16)%_nouns.length]) + '-' + id.substr(0,4)
} /* EOF */